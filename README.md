...
> Answer: Benefits of using client-side routing are that you dont necessarily have to wait for a server to respond with content, you have more control over the site URL and content, it's designed to mimic the appearance of traditional multi page websites without all of the excess reloading and re-downloading with every click.

- [x] What does AJAX stand for?
> Answer: Asynchronous JavaScript and XML

- [x] What are `controlled components` in React?
> Answer: An input form element whose value is controlled by React, when the React component that renders a form also controls what happens in that form on subsequent user input.
